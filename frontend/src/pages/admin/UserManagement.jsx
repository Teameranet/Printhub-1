...
const StatsRow = ({ users }) => {
  ...
  const regular = users.filter(u => u.userType === 'regular').length;
  const active = users.filter(u => u.status === 'active').length;
  const inactive = users.filter(u => u.status === 'suspended' || u.status === 'inactive').length;

  const stats = [
    { label: 'Total Users', value: total, color: 'var(--primary)', bg: 'var(--primary-soft)', border: 'var(--primary-border)', icon: <UsersIcon /> },
    { label: 'Regular Users', value: regular, color: '#2563eb', bg: 'rgba(37, 99, 235, 0.10)', border: 'rgba(37, 99, 235, 0.25)', icon: <UserIcon /> },
